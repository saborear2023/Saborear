...
public class FiltroGuiaView extends PagerAdapter {
    private final Activity context;
    private final ArrayList<String> nomes;
    private final Bitmap full, vazio;

    public FiltroGuiaView(Activity context, ArrayList<String> nomes) {
    ...
    public Object instantiateItem(@NonNull ViewGroup container, int position) {
        ...
        if(GuiaIngredientes.contains(receita)) {
            img.setImageBitmap(full);
            nome.setTextColor((Color.parseColor("#FFFFFF")));
            GuiaIngredientes.delImg(GuiaIngredientes.getImageByString(receita), receita, false);
            GuiaIngredientes.addImg(img, receita, false);
        } else {
        ...
        rowView.setOnClickListener(v -> {
            if(GuiaIngredientes.contains(receita)) {
                GuiaIngredientes.delImg(img, receita, true);
                img.setImageBitmap(vazio);
                nome.setTextColor((Color.parseColor("#1BAB4A")));
            } else {
                GuiaIngredientes.addImg(img, receita, true);
                img.setImageBitmap(full);
                nome.setTextColor((Color.parseColor("#FFFFFF")));
            }
        });

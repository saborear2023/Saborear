...
import Saborear.com.br.GuiaIngredientes;
import Saborear.com.br.R;
import Saborear.com.br.Utils.Utils;

public class OrdenarGuiaView extends PagerAdapter {
    ...
    public OrdenarGuiaView(Activity context, ArrayList<String> nomes) {
    ...
    public Object instantiateItem(@NonNull ViewGroup container, int position) {
        ...
        if(GuiaIngredientes.sel == position) {
        ...
        rowView.setOnClickListener(v -> {
            if(GuiaIngredientes.sel != position) {
                if(GuiaIngredientes.sel != -1) {
                    view.setImageBitmap(vazio);
                    text.setTextColor((Color.parseColor("#FD9F1C")));
                ...
                GuiaIngredientes.maior = true;
                GuiaIngredientes.sel = position;
            } else {
                ...
                if (!aux.contains("+") && !aux.contains("-")) {
                    ...
                    GuiaIngredientes.maior = false;
                    text.setText("- " + nomes.get(position));
                } else {
                    img.setImageBitmap(vazio);
                    nome.setTextColor((Color.parseColor("#FD9F1C")));
                    GuiaIngredientes.sel = -1;
                    text.setText(aux.replace("-", "").replace("+", ""));
                }
            }
            GuiaIngredientes.atualizar();
        });
